...
new Trigger({
  ...
  run: async (event, ctx) => {
    ...
    await slack.postMessage("New User", {
...
Run your server how you normally would, e.g. `npm run dev`. This will connect your workflow to Trigger.dev, so we can start sending you events. You should see some log messages in your server console (tip: you can turn these off by adding `logLevel: "disabled"` in the code above).
